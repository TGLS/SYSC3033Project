...
public class Server {
	...
	public void loop() {
		...
		while (true) {
			...
			// If validate request fails, print an error message and quit.
			// The specification says to raise an exception, which
			// leads to an absurd situation where an exception is caught
			// within the same try block it is raised.  
			if (!validateRequest()) {
